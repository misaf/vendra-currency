...
- Keep currency domain code inside `packages/vendra-currency` using the `Misaf\VendraCurrency` namespace.
...
- Keep Pest architecture tests in `tests/ArchTest.php`: the `php`, `security`, and `laravel` presets plus a tenant-agnostic expectation, e.g. `arch()->expect('Misaf\VendraCurrency')->not->toUse('Misaf\VendraTenant')`.
